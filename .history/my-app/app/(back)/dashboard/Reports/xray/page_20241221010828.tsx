...
const reports: Report[] = [
  { 
    id: 1, 
    category: 'X-ray', 
    title: 'X-ray Report 1', 
    content: 'Details of X-ray report 1',
    upid: 'P001', 
  ...
  { 
    id: 2, 
    category: 'X-ray', 
    title: 'X-ray Report 2', 
    content: 'Details of X-ray report 2',
    upid: 'P002', 
  ...
  { 
    id: 3, 
    category: 'X-ray', 
    title: 'X-ray Report 3', 
    content: 'Details of X-ray report 3',
    upid: 'P003', 
  ...
  { 
    id: 4, 
    category: 'X-ray', 
    title: 'X-ray Report 4', 
    content: 'Details of X-ray report 4',
    upid: 'P001', 
...
export default function XrayReportsPage() {
  // State to hold filtered reports by category "X-ray"
  const [filteredReports, setFilteredReports] = useState<Report[]>([]);
  const [selectedDate, setSelectedDate] = useState<string>('');

  useEffect(() => {
    // Filter reports by category "X-ray" and sort them by latest date and time
    const xrayReports = reports.filter(report => report.category === 'X-ray');
    const sortedReports = sortReportsByDate(xrayReports);
  ...
  const handleDateChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedDateValue) {
      ...
      const filteredByDate = reports.filter((report) => {
        ...
        return reportDateString === selectedDateValue && report.category === 'X-ray';
      });
      setFilteredReports(sortReportsByDate(filteredByDate)); // Sort by latest date and time
    } else {
      // If no date is selected, show all reports
      const xrayReports = reports.filter(report => report.category === 'X-ray');
      setFilteredReports(sortReportsByDate(xrayReports)); // Sort by latest date and time
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-green-50 to-green-100 flex items-center justify-center px-4">
      <div className="max-w-4xl w-full bg-white shadow-lg rounded-lg p-8">
        <h1 className="text-3xl font-extrabold text-gray-800 text-center mb-6">X-ray Reports</h1>
        ...
        {filteredReports.length > 0 ? (
            ...
            {filteredReports.map(report => (
              <div key={report.id} className="border p-4 rounded-lg shadow-md">
                ...
                <div className="mt-4 text-center">
                  <button 
                    onClick={() => alert(`Viewing X-ray report for UPID: ${report.upid}`)} 
          ...
          <p className="text-center text-lg text-gray-600">No X-ray reports available.</p>
        )}
      </div>
    </div>
  );
