...
const reports: Report[] = [
  { 
    id: 1, 
    category: 'Heart', 
    title: 'Heart Report 1', 
    content: 'Details of Heart report 1',
    upid: 'P001', 
  ...
  { 
    id: 2, 
    category: 'Heart', 
    title: 'Heart Report 2', 
    content: 'Details of Heart report 2',
    upid: 'P002', 
  ...
  { 
    id: 3, 
    category: 'Heart', 
    title: 'Heart Report 3', 
    content: 'Details of Heart report 3',
    upid: 'P003', 
  ...
  { 
    id: 4, 
    category: 'Heart', 
    title: 'Heart Report 4', 
    content: 'Details of Heart report 4',
    upid: 'P001', 
...
export default function HeartReportsPage() {
  // State to hold filtered reports by category "Heart"
  const [filteredReports, setFilteredReports] = useState<Report[]>([]);
  const [selectedDate, setSelectedDate] = useState<string>('');

  useEffect(() => {
    // Filter reports by category "Heart" and sort them by latest date and time
    const heartReports = reports.filter(report => report.category === 'Heart');
    const sortedReports = sortReportsByDate(heartReports);
  ...
  const handleDateChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedDateValue) {
      ...
      const filteredByDate = reports.filter((report) => {
        ...
        return reportDateString === selectedDateValue && report.category === 'Heart';
      });
      setFilteredReports(sortReportsByDate(filteredByDate)); // Sort by latest date and time
    } else {
      // If no date is selected, show all reports
      const heartReports = reports.filter(report => report.category === 'Heart');
      setFilteredReports(sortReportsByDate(heartReports)); // Sort by latest date and time
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-red-50 to-red-100 flex items-center justify-center px-4">
      <div className="max-w-4xl w-full bg-white shadow-lg rounded-lg p-8">
        <h1 className="text-3xl font-extrabold text-gray-800 text-center mb-6">Heart Reports</h1>
        ...
        {filteredReports.length > 0 ? (
          ...
          <p className="text-center text-lg text-gray-600">No Heart reports available.</p>
        )}
      </div>
    </div>
  );
