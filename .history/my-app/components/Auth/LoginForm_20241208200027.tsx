...
const LoginPage = () => {
  ...
  useEffect(() => {
    ...
    setRouter(useRouter()); // Set router only after mounting
  }, []);
  ...
  const onSubmit = async (data: LoginInputProps) => {
    ...
    try {
      ...
      if (!loginData || loginData.error) {
        ...
        router.push("/dashboard"); // Call router.push after mount
      }
    } catch (error) {
  ...
  if (!isMounted || !router) return null; // Only render after mounting and router is available
