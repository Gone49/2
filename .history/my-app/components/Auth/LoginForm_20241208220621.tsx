  ...
  const onSubmit = async (data: LoginInputProps) => {
    ...
    try {
      ...
      if (!loginData || loginData.error) {
        ...
        if (data.role === "USER") {
          router.push("/");
        } else if (data.role === "ADMIN" || data.role === "SERVICE_PROVIDER") {
...
// Updated mock implementation for `signIn`
async function signIn(
  ...
  if (
    options.userId === validCredentials.userId &&
    options.password === validCredentials.password &&
    options.role === validCredentials.role
  ) {
    return { error: false }; // Correct credentials
  } else {
    return { error: true }; // Incorrect credentials
  }
}
