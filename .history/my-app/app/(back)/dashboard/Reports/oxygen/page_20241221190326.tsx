...
const reports: Report[] = [
  { 
    id: 1, 
    category: 'Oxygen', 
    title: 'Oxygen Report 1', 
    content: 'Details of Oxygen report 1',
    upid: 'P001', 
  ...
  { 
    id: 2, 
    category: 'Oxygen', 
    title: 'Oxygen Report 2', 
    content: 'Details of Oxygen report 2',
    upid: 'P002', 
  ...
  { 
    id: 3, 
    category: 'Oxygen', 
    title: 'Oxygen Report 3', 
    content: 'Details of Oxygen report 3',
    upid: 'P003', 
  ...
  { 
    id: 4, 
    category: 'Oxygen', 
    title: 'Oxygen Report 4', 
    content: 'Details of Oxygen report 4',
    upid: 'P001', 
...
export default function OxygenReportsPage() {
  // State to hold filtered reports by category "Oxygen"
  const [filteredReports, setFilteredReports] = useState<Report[]>([]);
  const [selectedDate, setSelectedDate] = useState<string>('');

  useEffect(() => {
    // Filter reports by category "Oxygen" and sort them by latest date and time
    const oxygenReports = reports.filter(report => report.category === 'Oxygen');
    const sortedReports = sortReportsByDate(oxygenReports);
  ...
  const handleDateChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedDateValue) {
      ...
      const filteredByDate = reports.filter((report) => {
        ...
        return reportDateString === selectedDateValue && report.category === 'Oxygen';
      });
      setFilteredReports(sortReportsByDate(filteredByDate)); // Sort by latest date and time
    } else {
      // If no date is selected, show all reports
      const oxygenReports = reports.filter(report => report.category === 'Oxygen');
      setFilteredReports(sortReportsByDate(oxygenReports)); // Sort by latest date and time
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-blue-50 to-blue-100 flex items-center justify-center px-4">
      <div className="max-w-4xl w-full bg-white shadow-lg rounded-lg p-8">
        <h1 className="text-3xl font-extrabold text-gray-800 text-center mb-6">Oxygen Reports</h1>
        ...
        {filteredReports.length > 0 ? (
            ...
            {filteredReports.map(report => (
              <div key={report.id} className="border p-4 rounded-lg shadow-md">
                ...
                <div className="mt-4 text-center">
                  <button 
                    onClick={() => alert(`Viewing Oxygen report for UPID: ${report.upid}`)} 
          ...
          <p className="text-center text-lg text-gray-600">No Oxygen reports available.</p>
        )}
      </div>
    </div>
  );
