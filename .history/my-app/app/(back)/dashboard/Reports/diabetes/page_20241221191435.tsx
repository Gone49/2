...
const reports: Report[] = [
  { 
    id: 1, 
    category: 'Diabetes', 
    title: 'Diabetes Report 1', 
    content: 'Details of diabetes report 1',
    upid: 'P001', 
  ...
  { 
    id: 2, 
    category: 'Diabetes', 
    title: 'Diabetes Report 2', 
    content: 'Details of diabetes report 2',
    upid: 'P002', 
  ...
  { 
    id: 3, 
    category: 'Diabetes', 
    title: 'Diabetes Report 3', 
    content: 'Details of diabetes report 3',
    upid: 'P003', 
  ...
  { 
    id: 4, 
    category: 'Diabetes', 
    title: 'Diabetes Report 4', 
    content: 'Details of diabetes report 4',
    upid: 'P001', 
...
export default function DiabetesReportsPage() {
  // State to hold filtered reports by category "Diabetes"
  const [filteredReports, setFilteredReports] = useState<Report[]>([]);
  const [selectedDate, setSelectedDate] = useState<string>('');

  useEffect(() => {
    // Filter reports by category "Diabetes" and sort them by latest date and time
    const diabetesReports = reports.filter(report => report.category === 'Diabetes');
    const sortedReports = sortReportsByDate(diabetesReports);
  ...
  const handleDateChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedDateValue) {
      ...
      const filteredByDate = reports.filter((report) => {
        ...
        return reportDateString === selectedDateValue && report.category === 'Diabetes';
      });
      setFilteredReports(sortReportsByDate(filteredByDate)); // Sort by latest date and time
    } else {
      // If no date is selected, show all reports
      const diabetesReports = reports.filter(report => report.category === 'Diabetes');
      setFilteredReports(sortReportsByDate(diabetesReports)); // Sort by latest date and time
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r flex items-center justify-center px-4">
      <div className="max-w-4xl w-full bg-white shadow-lg rounded-lg p-8">
        <h1 className="text-3xl font-extrabold text-gray-800 text-center mb-6">Diabetes Reports</h1>
        ...
        {filteredReports.length > 0 ? (
          ...
          <p className="text-center text-lg text-gray-600">No diabetes reports available.</p>
        )}
      </div>
    </div>
  );
