...
const reports: Report[] = [
  { 
    ...
    category: 'ECG', 
    title: 'ECG Report 1', 
    content: 'Details of ECG report 1',
    upid: 'P001', 
  ...
  { 
    ...
    category: 'ECG', 
    title: 'ECG Report 2', 
    content: 'Details of ECG report 2',
    upid: 'P002', 
  ...
  { 
    ...
    category: 'ECG', 
    title: 'ECG Report 3', 
    content: 'Details of ECG report 3',
    upid: 'P003', 
  ...
  { 
    ...
    category: 'ECG', 
    title: 'ECG Report 4', 
    content: 'Details of ECG report 4',
    upid: 'P001', 
...
export default function ECGReportsPage() {
  // State to hold filtered reports by category "ECG"
  const [filteredReports, setFilteredReports] = useState<Report[]>([]);
  const [selectedDate, setSelectedDate] = useState<string>('');

  useEffect(() => {
    // Filter reports by category "ECG" and sort them by latest date and time
    const ecgReports = reports.filter(report => report.category === 'ECG');
    const sortedReports = sortReportsByDate(ecgReports);
  ...
  const handleDateChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedDateValue) {
      ...
      const filteredByDate = reports.filter((report) => {
        ...
        return reportDateString === selectedDateValue && report.category === 'ECG';
      });
      setFilteredReports(sortReportsByDate(filteredByDate)); // Sort by latest date and time
    } else {
      // If no date is selected, show all reports
      const ecgReports = reports.filter(report => report.category === 'ECG');
      setFilteredReports(sortReportsByDate(ecgReports)); // Sort by latest date and time
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-green-50 to-green-100 flex items-center justify-center px-4">
      <div className="max-w-4xl w-full bg-white shadow-lg rounded-lg p-8">
        <h1 className="text-3xl font-extrabold text-gray-800 text-center mb-6">ECG Reports</h1>
        ...
        {filteredReports.length > 0 ? (
          ...
          <p className="text-center text-lg text-gray-600">No ECG reports available.</p>
        )}
      </div>
    </div>
  );
