...
import { signIn, SignInResponse } from "next-auth/react"; // Import SignInResponse directly
  ...
  const onSubmit = async (data: LoginInputProps) => {
    ...
    try {
      ...
      if (loginData?.error) {
        ...
        const userRole = loginData?.user?.role; // Assuming the role is available here
        if (userRole === "USER") {
          router.push("/"); // Redirect to the home page for users
        } else if (userRole === "ADMIN" || userRole === "SERVICE_PROVIDER") {
          router.push("/dashboard"); // Redirect to the dashboard for admins and service providers
        } else {
