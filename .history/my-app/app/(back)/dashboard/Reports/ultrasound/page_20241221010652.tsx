...
const reports: Report[] = [
  { 
    id: 1, 
    category: 'Ultrasound', 
    title: 'Ultrasound Report 1', 
    content: 'Details of Ultrasound report 1',
    upid: 'P001', 
  ...
  { 
    id: 2, 
    category: 'Ultrasound', 
    title: 'Ultrasound Report 2', 
    content: 'Details of Ultrasound report 2',
    upid: 'P002', 
  ...
  { 
    id: 3, 
    category: 'Ultrasound', 
    title: 'Ultrasound Report 3', 
    content: 'Details of Ultrasound report 3',
    upid: 'P003', 
  ...
  { 
    id: 4, 
    category: 'Ultrasound', 
    title: 'Ultrasound Report 4', 
    content: 'Details of Ultrasound report 4',
    upid: 'P001', 
...
export default function UltrasoundReportsPage() {
  // State to hold filtered reports by category "Ultrasound"
  const [filteredReports, setFilteredReports] = useState<Report[]>([]);
  const [selectedDate, setSelectedDate] = useState<string>('');

  useEffect(() => {
    // Filter reports by category "Ultrasound" and sort them by latest date and time
    const ultrasoundReports = reports.filter(report => report.category === 'Ultrasound');
    const sortedReports = sortReportsByDate(ultrasoundReports);
  ...
  const handleDateChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedDateValue) {
      ...
      const filteredByDate = reports.filter((report) => {
        ...
        return reportDateString === selectedDateValue && report.category === 'Ultrasound';
      });
      setFilteredReports(sortReportsByDate(filteredByDate)); // Sort by latest date and time
    } else {
      // If no date is selected, show all reports
      const ultrasoundReports = reports.filter(report => report.category === 'Ultrasound');
      setFilteredReports(sortReportsByDate(ultrasoundReports)); // Sort by latest date and time
    }
  };

  return (
    <div className="min-h-screen bg-gradient-to-r from-green-50 to-green-100 flex items-center justify-center px-4">
      <div className="max-w-4xl w-full bg-white shadow-lg rounded-lg p-8">
        <h1 className="text-3xl font-extrabold text-gray-800 text-center mb-6">Ultrasound Reports</h1>
        ...
        {filteredReports.length > 0 ? (
            ...
            {filteredReports.map(report => (
              <div key={report.id} className="border p-4 rounded-lg shadow-md">
                ...
                <div className="mt-4 text-center">
                  <button 
                    onClick={() => alert(`Viewing Ultrasound report for UPID: ${report.upid}`)} 
                    className="px-6 py-2 bg-gradient-to-r from-green-500 to-green-600 text-white font-semibold rounded-lg shadow-lg hover:from-green-600 hover:to-green-700 transition duration-300"
                  >
                    View Report
                  </button>
                </div>
          ...
          <p className="text-center text-lg text-gray-600">No Ultrasound reports available.</p>
        )}
      </div>
    </div>
  );
