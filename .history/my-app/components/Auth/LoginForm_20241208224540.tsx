...
const LoginPage = () => {
  ...
  useEffect(() => {
    if (status === "authenticated") {
      const userRole = session?.user?.role;  // Access user role
      if (userRole === "USER") {
        router.push("/"); // Redirect to home page for 'USER'
      } else if (userRole === "ADMIN" || userRole === "SERVICE_PROVIDER") {
        router.push("/dashboard"); // Redirect to dashboard for 'ADMIN' or 'SERVICE_PROVIDER'
      }
    }
  ...
  const onSubmit = async (data: LoginInputProps) => {
    ...
    try {
      ...
      // Handling loginData response
      if (loginData?.error) {
        toast.error("Sign-in error: " + loginData.error);
      } else if (loginData?.ok) {
        toast.success("Login Successful");
        // After login, session will be available, and redirection will happen based on role
      } else {
