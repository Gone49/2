...
const LoginPage = () => {
  ...
  const [loginError, setLoginError] = useState(false);  // Track if there's a login error
  ...
  const onSubmit = async (data: LoginInputProps) => {
    ...
    setLoginError(false); // Reset error before trying to log in
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative">
      ...
      <div className="relative z-10 bg-white/95 p-6 rounded-xl shadow-lg backdrop-blur-sm max-w-sm w-full lg:max-w-md lg:backdrop-blur-md">
        ...
        {/* Show Alert if there's a login error */}
        {loginError && (
