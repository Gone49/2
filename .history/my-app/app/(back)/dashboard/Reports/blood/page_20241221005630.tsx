...
export default function AsthmaReportsPage() {
  ...
  const handleDateChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (date) {
      ...
      const reportsByDate = reports.filter(report => report.dateGenerated.startsWith(date) && report.category === 'Asthma');
      setFilteredReports(sortReportsByDate(reportsByDate)); // Sort by date and time in descending order
    } else {
      // If no date is selected, show all reports
      const asthmaReports = reports.filter(report => report.category === 'Asthma');
      setFilteredReports(sortReportsByDate(asthmaReports)); // Sort by date and time in descending order
    }
  };
