...
const reports: Report[] = [
  { 
    id: 1, 
    category: 'CT Scan', 
    title: 'CT Scan Report 1', 
    content: 'Details of CT scan report 1',
    upid: 'P001', 
  ...
  { 
    id: 2, 
    category: 'CT Scan', 
    title: 'CT Scan Report 2', 
    content: 'Details of CT scan report 2',
    upid: 'P002', 
  ...
  { 
    id: 3, 
    category: 'CT Scan', 
    title: 'CT Scan Report 3', 
    content: 'Details of CT scan report 3',
    upid: 'P003', 
  ...
  { 
    id: 4, 
    category: 'CT Scan', 
    title: 'CT Scan Report 4', 
    content: 'Details of CT scan report 4',
    upid: 'P001', 
...
export default function CTScanReportsPage() {
  // State to hold filtered reports by category "CT Scan"
  const [filteredReports, setFilteredReports] = useState<Report[]>([]);
  const [selectedDate, setSelectedDate] = useState<string>('');

  useEffect(() => {
    // Filter reports by category "CT Scan" and sort them by latest date and time
    const ctScanReports = reports.filter(report => report.category === 'CT Scan');
    const sortedReports = sortReportsByDate(ctScanReports);
  ...
  const handleDateChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedDateValue) {
      ...
      const filteredByDate = reports.filter((report) => {
        ...
        return reportDateString === selectedDateValue && report.category === 'CT Scan';
      });
      setFilteredReports(sortReportsByDate(filteredByDate)); // Sort by latest date and time
    } else {
      // If no date is selected, show all reports
      const ctScanReports = reports.filter(report => report.category === 'CT Scan');
      setFilteredReports(sortReportsByDate(ctScanReports)); // Sort by latest date and time
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-green-50 to-green-100 flex items-center justify-center px-4">
      <div className="max-w-4xl w-full bg-white shadow-lg rounded-lg p-8">
        <h1 className="text-3xl font-extrabold text-gray-800 text-center mb-6">CT Scan Reports</h1>
        ...
        {filteredReports.length > 0 ? (
          ...
          <p className="text-center text-lg text-gray-600">No CT scan reports available.</p>
        )}
      </div>
    </div>
  );
