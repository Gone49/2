...
const reports: Report[] = [
  { 
    ...
    category: 'MRI', 
    title: 'MRI Report 1', 
    content: 'Details of MRI report 1',
    upid: 'P001', 
  ...
  { 
    ...
    category: 'MRI', 
    title: 'MRI Report 2', 
    content: 'Details of MRI report 2',
    upid: 'P002', 
  ...
  { 
    ...
    category: 'MRI', 
    title: 'MRI Report 3', 
    content: 'Details of MRI report 3',
    upid: 'P003', 
  ...
  { 
    ...
    category: 'MRI', 
    title: 'MRI Report 4', 
    content: 'Details of MRI report 4',
    upid: 'P001', 
...
export default function MRIReportsPage() {
  // State to hold filtered reports by category "MRI"
  const [filteredReports, setFilteredReports] = useState<Report[]>([]);
  const [selectedDate, setSelectedDate] = useState<string>('');

  useEffect(() => {
    // Filter reports by category "MRI" and sort them by latest date and time
    const mriReports = reports.filter(report => report.category === 'MRI');
    const sortedReports = sortReportsByDate(mriReports);
  ...
  const handleDateChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedDateValue) {
      ...
      const filteredByDate = reports.filter((report) => {
        ...
        return reportDateString === selectedDateValue && report.category === 'MRI';
      });
      setFilteredReports(sortReportsByDate(filteredByDate)); // Sort by latest date and time
    } else {
      // If no date is selected, show all reports
      const mriReports = reports.filter(report => report.category === 'MRI');
      setFilteredReports(sortReportsByDate(mriReports)); // Sort by latest date and time
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-green-50 to-green-100 flex items-center justify-center px-4">
      <div className="max-w-4xl w-full bg-white shadow-lg rounded-lg p-8">
        <h1 className="text-3xl font-extrabold text-gray-800 text-center mb-6">MRI Reports</h1>
        ...
        {filteredReports.length > 0 ? (
            ...
            {filteredReports.map(report => (
              <div key={report.id} className="border p-4 rounded-lg shadow-md">
                ...
                <div className="mt-4 text-center">
                  <button 
                    onClick={() => alert(`Viewing MRI report for UPID: ${report.upid}`)} 
          ...
          <p className="text-center text-lg text-gray-600">No MRI reports available.</p>
        )}
      </div>
    </div>
  );
